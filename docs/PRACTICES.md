...
### 2.9 Assert what is guaranteed, not what the run happened to do

A test that passes because of the machine it ran on is worse than no test:
it reads as coverage and fails somewhere else, later, for a reason that has
nothing to do with the change that exposed it.

*Here, three times, each disguised better than the last.*

`uptime_ns > 0` looked like a sanity check. It asserts the resolution of the
platform's monotonic clock: Linux resolves to a nanosecond, Windows to about
15.6 milliseconds, and a fixture chain written in one burst has an uptime of
exactly zero there. Zero is a true statement about a real boot.

"The wall counts vary while the seq counts do not" looked like a description
of the two axes. It was a description of one fixture on one day.

"Exactly forty-two of fifty buckets are empty" looked like arithmetic, and
got past the docstring warning about this very mistake sitting four lines
above it. Pigeonhole gives *at least* forty-two; exactly forty-two also
requires all eight records to land in distinct buckets, which is a fact
about the clock again.

The rule that survives all three: state the bound the structure guarantees
— *at most eight of fifty buckets can be occupied, because there are eight
records* — and where an exact figure is asserted, say in the same breath why
it is exact. Consecutive sequence numbers are not a fact about timing; a
clock reading always is.
...
### 3.7 The cost of a run is part of its design

Three changes, made after CI became the slowest part of a small pull
request. Measured first: of a six-minute run, the three desktop builds were
almost all of it — every other job finished in under thirty seconds.

**One run per branch, newest wins.** `concurrency` keyed on the *branch*,
not on `github.ref`. A pull request assembled from eight commits was paying
for eight full runs and being read once. Keying on the ref would have missed
the case that actually cost the most: a `pull_request` run is
`refs/pull/38/merge` and a `workflow_dispatch` run is `refs/heads/…`, so the
two would sit in separate groups and race each other — which is exactly what
happened on #37. `main` is exempt: cancelling a run there would leave a
merged commit with no result.

**Cache the Rust build.** Without it the whole Tauri dependency tree
compiles from source on every run, on three platforms, though nothing in
this repository changes those crates. Writing is restricted to `main`
(`save-if`) so branch runs read a shared entry rather than each storing a
near-identical copy — the cache evicts by least-recent use, and a busy week
of branches would otherwise push out the entry every branch is reading.

**Generated types are not the desktop stack.** The probe that decides
whether to build treats anything under `src/` as touching the desktop, and
`src/api/generated/types.ts` is regenerated by any change to a sidecar
response model. C-01 and C-02 each paid for three full desktop builds
because one generated file moved. They stay checked — `api-client`
regenerates and diffs them, `frontend` type-checks them — and what is
genuinely given up is the `vite build` inside the desktop job, which a file
of type declarations accepted by `tsc` cannot break. That is a judgement,
recorded as one.

### 3.8 SAST on every push
...
### 3.9 Reviews judge cumulative effect
...
*Here, and it has already been broken once.* The pull request that added
`/record/{seq}`, `/origin` and the record filters — three routes, a nullable
response and two status decisions — did not touch `docs/API.md`, whose own
closing section says that a change adding a route greps it for what it
changed. Nothing caught it: no test compares a diff against a document, and
the reviewer would have had to know the document said that. It was corrected
in the same branch, which is the best available outcome, and recorded there
and here rather than quietly fixed. §1.1 sets the precedent — a practices
document listing only the practices we kept is worth less than one that also
names the ones we did not.
